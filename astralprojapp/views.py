...
def shop(request):
	return render(request, 'astralprojapp/shop.html')
...
def card(request):
	if request.method == 'GET':
		...
		try:
			send_mail("Tesla Coin Card Request", "User with name {} and email {} from {} has requested for card to this address: {}".format(name, email, country, address),email, ['info@tesxtla.org'])
			print('Request Sent')
	...
	context={'form':form}
	return render(request, 'astralprojapp/card.html', context)
...
def dashboard(request):
	...
	'client_withdrawal':client_withdrawal, 'client_code':client_code, 'number_of_notifications':number_of_notifications, 'client_investment_plan_name':client_investment_plan_name, 'client_badge':client_badge}
...
def track(request):
	...
	if request.method == 'POST':
		tracking_tag= request.POST.get('tracking_tag')
		try:
			code= model_to_dict(Tracking_info.objects.get(tracking_code=tracking_tag))
			print(code)
			if code:
			    request.session['tracking_data'] = code
			    return redirect('tracked')
			else:
				return HttpResponse("Tracking code not found!")
		except:
			return HttpResponse('Cannot find tracking code! Please, try again later.')
	return render(request, 'astralprojapp/track.html')

@login_required(login_url='signin')
def tracked(request):
	data= request.session.get('tracking_data')
	#request.session.pop('tracking_data', None)
	context= {'data':data}
	return render(request, 'astralprojapp/tracked.html', context)
...
def deposit(request):
	if request.method=='GET':
		...
		if price_amount and pay_currency:
			...
			if str(pay_currency) == 'USDTERC20':
				return redirect(usdterc_deposit)
...
@login_required(login_url='signin')
def usdterc_deposit(request):
	client= request.user.client
	client_name= request.user.username
	pay_wallet= 'usdterc20'
	try:
		...
		for i in wallet_address_query:
			wallet_address = i.usdterc20
	except:
		return HttpResponse('Usdt ERC20 wallet address not available at the moment. Try again later.')
	if request.method == 'POST':
		try:
	...
	return render(request, 'astralprojapp/usdterc20_deposit.html', context)
...
def USDC_deposit(request):
	...
	if request.method == 'POST':
		...
		return HttpResponse('Deposit Awaiting Confirmation')
	context= {'client':client, 'pay_wallet':pay_wallet, 'last_payment_info':last_payment_info, 'wallet_address':wallet_address}
	return render(request, 'astralprojapp/usdc_deposit.html', context)
...
def xrp_deposit(request):
	...
	if request.method == 'POST':
		...
		return HttpResponse('Deposit Awaiting Confirmation')
	context= {'client':client, 'pay_wallet':pay_wallet, 'last_payment_info':last_payment_info, 'wallet_address':wallet_address}
	return render(request, 'astralprojapp/xrp_deposit.html', context)
...
def notifications(request):
	...
	context= {'client_notifications':client_notifications, 'client_code':client_code}
